...
export default App.CuratedContentEditorLabelsMixin = Ember.Mixin.create({
	...
	itemsCountLabel: Ember.computed('model.items.length', function () {
		return i18next.t('app.curated-content-editor-items-count', {count: this.get('model.items.length')});
	}),
	...
	headerLabel: Ember.computed('model.label', 'isFeatured', 'isSection', function () {
		...
		if (modelLabel) {
			...
			return i18next.t('app.curated-content-editor-new-featured-content');
		} else if (this.get('isSection')) {
			return i18next.t('app.curated-content-editor-new-section');
		}
		return i18next.t('app.curated-content-editor-new-category');
	}),

	pageNameLabel: Ember.computed('isCategory', function () {
		if (this.get('isCategory')) {
			return i18next.t('app.curated-content-editor-enter-category-name');
		}
		return i18next.t('app.curated-content-editor-enter-page-name');
	})
});
