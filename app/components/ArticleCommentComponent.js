...
App.ArticleCommentComponent = Em.Component.extend({
    ...
    convertThumbnails: function ($originalFigure) {
        ...
        thumbnailsData.forEach(function (thumbnailData) {
            ...
            }), $thumbnail = $('<img/>').attr('src', thumbnailURL), href = '%@%@:%@'.fmt(Em.get(Mercury, 'wiki.articlePath'), Em.getWithDefault(Mercury, 'wiki.namespaces.6', 'File'), thumbnailData.name), $anchor = $('<a/>').attr('href', href).append($thumbnail), $figcaption, $figure = $('<figure/>');
